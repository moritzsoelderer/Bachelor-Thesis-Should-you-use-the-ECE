...
import os
import src.metrics.ksce_utilities as utils

def ksce(scores, labels):
    scores = ensure_numpy(scores)
    labels = ensure_numpy(labels)

    ### Added by myself so that I am not obligated to use a 1d array with positive class probabilities
    ...
    test = np.absolute(integrated_scores - integrated_accuracy)
...
def plot_KS_graphs(scores, labels, spline_method, splines, outdir, plotname, title="", showplots=True):
    ...
    fitted_accuracy, fitted_error = compute_accuracy(scores, labels, spline_method, splines, outdir, plotname,
                                                     showplots=showplots)
