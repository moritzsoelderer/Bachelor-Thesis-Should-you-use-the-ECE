...
def calculate_probabilities(samples, step):
    # noise = stats.norm(loc=0, scale=0.05).rvs(size=samples.shape[0])
    prob_dist = stats.norm(loc=0, scale=step)
    scaling_constant = step * np.sqrt(2 * np.pi)
    # pdf_values = prob_dist.pdf(samples) * scaling_constant + noise
...
def main():
    ...
    util.plot_pred_prob_dists(np.arange(variance + max_negative_variance_deviation,
    ...
    for variance_deviation in variance_deviations:
        ...
        pred_prob = calculate_probabilities(samples, variance + variance_deviation)
    ...
    util.plot_metrics(variance, variance + variance_deviations, true_prob, true_labels, true_ece_vals, ece_vals, balance_score_vals,
