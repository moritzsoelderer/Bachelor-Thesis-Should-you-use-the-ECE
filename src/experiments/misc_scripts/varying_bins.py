...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"./logs/varying_bins_dataset_family/{datetime_start.strftime('%Y%m%d_%H%M%S')}.log"),  # Log to file
...
def main():
    ...
    for model_name, model_pred_fun_tuple in models.items():
        ...
        with open('./data/varying_bins_dataset_family/' + filename_absolute + '.pkl', 'wb') as file:
        ...
        plt.savefig("./plots/varying_bins_dataset_family/" + filename_absolute + ".png")
        ...
        plt.savefig("./plots/varying_bins_dataset_family/" + filename_relative + ".png")
        plt.show(block=False)
