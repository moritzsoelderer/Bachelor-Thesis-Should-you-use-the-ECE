...
def run(dataset_name, dataset_size, min_samples, max_samples, num_steps, true_ece_sample_size):
    ...
    models = train_models(train_samples, train_labels,  sample_dim=data_generation.n_features)
    ...
    for model_name, model_pred_fun_tuple in models.items():
        ...
        # Sanity Check
        #assert len(true_ece_samples_dists) == len(estimators) == len(true_probabilities_grid)
        ...
        true_ece_values_15_bins = [
            calculate_true_ece_on_dists_and_grid(
                data_generation, true_ece_samples_dists,
                estimator, model_pred_fun_tuple[1], true_ece_samples_grid,
                true_probabilities_grid, 15) for index, estimator in enumerate(estimators)
        ]
        true_ece_values_100_bins = [
            calculate_true_ece_on_dists_and_grid(
                data_generation, true_ece_samples_dists,
                estimator, model_pred_fun_tuple[1], true_ece_samples_grid,
                true_probabilities_grid, 100) for index, estimator in enumerate(estimators)
        ]
        ...
        logging.info(
            ...
            model_name, data_generation.title, sampless_and_labelss[0][0].shape, sampless_and_labelss[0][1].shape, 1
        )
        ...
        ### Data Persistence
        persist_to_pickle(estimators, true_ece_samples_dists, true_ece_samples_grid, true_probabilities_grid, means, std_devs, subsample_sizes, savePath)
